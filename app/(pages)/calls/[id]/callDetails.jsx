...
//   console.log("Parts of the string:", parts);

//   const replacements = {};

//   // Iterate over each entity and find a corresponding replacement in the transcription
//   entities.forEach((entity, index) => {
//     // Check if the entity exists in entityRules.entities[0] and its value is false
//     const entityKey = `[${entity}]`; // Construct the key (e.g., "[ORGANISATION]")
//     const shouldReplace = entityRules.entities[entityKey] === false;

//     console.log(`Entity: ${entity}, Should Replace: ${shouldReplace}`);

//     if (shouldReplace) {
//       // Find the context before and after the current entity
...
//       // Try to find a suitable replacement in the full transcription
//       const beforeTrim = beforeContext.trim().split(/\s+/).pop() || '';
//       const afterTrim = afterContext.trim().split(/\s+/).slice(0, 2).join(' ') || '';

//       // We will now attempt to replace all occurrences, not just the first one
//       let startIndex = transcription.indexOf(beforeTrim);

//       if (startIndex === -1 && afterTrim) {
//         // If the first context doesn't work, try with the after context
//         startIndex = transcription.indexOf(afterTrim);
//       }

//       if (startIndex !== -1) {
//         // Look for the next occurrence of the after context
...
//           if (replacementText) {
//             // Store the replacement for this entity
//             replacements[entity] = replacementText;
//           }
//         }
//       }
//     } else {
//       console.log(`Skipping replacement for entity: ${entity} (value is true)`);
//     }
//   });

//   // Now, replace all occurrences of the entities in the original redaction statement
...
//     if (shouldReplace) {
//       // Replace the entity with its corresponding replacement
//       result = result.replace(new RegExp(`\\[${entity}\\]`, 'g'), replacements[entity] || `[${entity}]`);
//     } else {
//       // Skip replacement if the entity's value is true
//       console.log(`Entity [${entity}] not replaced (value is true)`);
//     }
//   });
...
export default function CallInfo({ call, organisations, url }) {
  ...
  const [showTranslation, setShowTranslation] = useState(true);
  const [search, setSearch] = useState("");
  const [showRedacted, setShowRedacted] = useState(true); // New state for toggling redaction
